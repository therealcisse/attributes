...
trait Attributes {
  ...
  def size: Int
  def isEmpty: Boolean
  def asMap: Map[AttributeKey[_], Any]
  def add[T](key: String, value: T)(implicit evidence: AttributeKey.ForAttributeType[T]): Attributes
}

object Attributes {

  private class AttributesImpl private[Attributes] (data: Array[Any]) extends Attributes {
    private val underlying = ImmutableKeyValuePairs[AttributeKey[_]](data)

    def get[T](key: AttributeKey[T]): Option[T] = underlying.get(key)
    def size: Int = underlying.size
    def isEmpty: Boolean = underlying.isEmpty
    def asMap: Map[AttributeKey[_], Any] = underlying.asMap
  ...
  def empty: Attributes = {
    new AttributesImpl(Array.empty[Any])
  }

  def of[T](key: String, value: T)(implicit evidence: AttributeKey.ForAttributeType[T]): Attributes = {
    empty.add(key, value)
  }

  def apply(pairs: (AttributeKey[_], Any)*): Attributes = {
    if (pairs.isEmpty) {
      empty
    } else {
  ...
  def fromBytes(bytes: Array[Byte]): Either[String, Attributes] = {
    Attributes.Serialization.protobufCodec.decode(zio.Chunk.fromArray(bytes))
  ...
  private object Serialization {
    ...
    private def valueToAttributeValue(value: Any, attributeType: AttributeType): AttributeValue = attributeType match {
    ...
    private def attributeValueToValue(attributeValue: AttributeValue): Any = attributeValue match {
    ...
    def toSerializable(attributes: Attributes): List[SerializableAttribute] = {
      attributes.asMap.map { case (key, value) =>
        SerializableAttribute(key.key, key.typ, valueToAttributeValue(value, key.typ))
      }.toList
    }
